...
def test_b4_linear_head_of_recommended_next_providers() -> None:
    ...
    assert ("linear" in provider.lower() or "Linear" in str(first)
        ...
        f"Linear, Jira, GitLab, Terraform, or Kubernetes should be head; got: {provider!r}"
    )
...
def test_b5_expansion_framework_next_milestone_m85a() -> None:
    ...
    assert ("M85A" in next_ms or "Linear" in next_ms
        ...
        f"next_milestone should reference M85A/Linear through M89A/Kubernetes; got: {next_ms!r}"
    )
...
def test_b6_expansion_framework_next_provider_linear() -> None:
    ...
    assert ("Linear" in next_prov or "linear" in next_prov.lower()
        ...
        f"next_provider should be Linear, Jira, GitLab, Terraform, Kubernetes, or Sentry; got: {next_prov!r}"
    )
...
def test_d15_linear_is_head_of_next_providers_brief() -> None:
    ...
    if block_start == -1 or block_end == -1:
        return
    brief_block = text[block_start:block_end]
    assert ("Linear" in brief_block or "Jira" in brief_block or "Terraform" in brief_block or "GitLab" in brief_block or "Kubernetes" in brief_block), (
        "NEXT_PROVIDERS_BRIEF should contain Linear, Jira, GitLab, Terraform, or Kubernetes after PagerDuty arc complete"
    )
