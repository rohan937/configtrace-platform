...
class TestSectionH_CapabilityMatrixAndExpansionFramework:
    ...
    def test_capability_matrix_clerk_signals_false(self):
        ...
        assert clerk_cap.security.risk_activity_correlations is False, (
            f"risk_activity_correlations must be False for clerk at M83D. "
            f"Got: {clerk_cap.security.risk_activity_correlations!r}"
        )
        assert clerk_cap.security.demo_seed_clear is False, (
            f"demo_seed_clear must be False for clerk at M83D. "
    ...
    def test_expansion_framework_m83e(self):
        ...
        assert any(
            term in planned
            for term in ("M83E", "Activity Signals", "Clerk")
        ), (
            f"planned_next_stage must reference M83E/Activity Signals/Clerk, got: {planned!r}"
        )
