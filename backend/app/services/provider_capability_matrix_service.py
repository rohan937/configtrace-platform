...
_PAGERDUTY = ProviderCapability(
    ...
    notes=(
        ...
        "demo + QA, and provider depth QA (M84A-M84H). Drift snapshots cover 8 safe "
        "configuration surfaces from the PagerDuty REST API v2: services, escalation "
        ...
        "security rules; M84C adds 18 expanded rules (40 total). M84D adds review-safe "
        "configuration activity ingestion from the same 8 drift surfaces. M84E "
        "promotes activity events into review-priority Incident Signals (9 signal "
        ...
        "taxonomy parity (8 record types, 40 rule keys, 9 activity event types, 9 "
        "signal types, 8 correlation types), privacy/sanitization discipline on the "
        "live signal/correlation/demo pipeline, claim discipline across all PagerDuty "
        "modules, false-positive behavior for rule evaluation, demo isolation across "
        "providers, router admin guards, and frontend consistency. Fixed spurious "
        "pagerduty_config_activity_correlation in frontend correlations page. Added "
        "PagerDuty to securityDemoScript.ts talk tracks. "
        ...
        "planned_next_stage: M84I: PagerDuty Cross-Cloud UX Polish."
    ),
)
