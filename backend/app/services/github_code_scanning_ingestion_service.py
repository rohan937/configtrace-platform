"""GitHub code-scanning alert ingestion (M69.4D).

Ingests GitHub repository CODE-SCANNING ALERTS (the
``GET /repos/{owner}/{repo}/code-scanning/alerts`` API) into the shared
``security_activity_events`` table (``provider="github"``,
``source="code_scanning_alert"``) — GitHub's SECOND security-alert evidence plane
in ConfigTrace (after secret-scanning alerts in M69.4A). The existing audit-log
source is control-plane activity; this is provider-reported SAST alert evidence.
...
code-scanning ingestion (``GET /orgs/{org}/code-scanning/alerts``) is DEFERRED —
it needs org-admin scope that the repo-scoped credential model does not
guarantee.

CLAIM DISCIPLINE: these are provider-reported ALERTS for review. ConfigTrace
never asserts that a vulnerability was exploited, that a compromise occurred,
that an attacker is present, or that unauthorized access happened — only
"evidence for review". An open alert is an alert, not a confirmed incident, and
ConfigTrace does not confirm exploitability or exploitation.

NON-FATAL BY DESIGN: every failure (GitHub Advanced Security / code scanning
disabled, missing permission/scope, private repo unavailable, 403/404, rate
limit, malformed response) is captured in the returned summary — never raised out
to break a normal GitHub sync.

PRIVACY: only allowlisted, flat, safe fields are stored. NEVER raw SARIF, raw
code snippets, raw file contents, raw alert locations/paths, the raw alert URL
(hashed only), the raw API response, request/response headers, commit/patch
content, tokens, or credentials. The activity allowlist is the final gate.
"""
...
EVENT_SOURCE = "github_code_scanning"

# Map alert ``state`` → normalized event type. GitHub's code-scanning alert state
# enum is open / dismissed / fixed; ``reopened`` is included for completeness.
_STATE_MAP: dict[str, str] = {
    "open": "github.code_scanning.alert.open",
    "fixed": "github.code_scanning.alert.fixed",
    "dismissed": "github.code_scanning.alert.dismissed",
    "reopened": "github.code_scanning.alert.reopened",
}
_FALLBACK_EVENT_TYPE = "github.code_scanning.alert.event"
...
def _url_hash(value: Any) -> Optional[str]:
    ...
    salt = ("gh_cs_url_v1:" + str(key)).encode("utf-8")
...
def _instances_count(alert: dict[str, Any]) -> Optional[int]:
    """Safe COUNT of alert instances if embedded. Never the raw instances/locations."""
    inst = alert.get("instances")
...
def normalize_code_scanning_alert(
    ...
    """Map one raw code-scanning alert → a normalized activity dict, or None.

    Reads only safe, flat fields. NEVER the raw SARIF, code snippet, file
    contents, raw locations/paths, or the raw alert URL.
    """
    ...
    pe_id = "ghcs:" + hashlib.sha256(pe_basis.encode("utf-8")).hexdigest()[:40]
    ...
    metadata = {
        ...
        # Short, safe rule description only (sanitizer truncates to 200 chars).
        # NEVER the long full_description or any instance message/code text.
        "rule_description": _safe_str(rule.get("description")),
        "tool_name": _safe_str(tool.get("name")),
        "severity": _safe_str(rule.get("severity")),
        "security_severity_level": _safe_str(rule.get("security_severity_level")),
        ...
        "instances_count": _instances_count(alert),
    ...
    return activity_svc.normalize_activity_event(
        ...
        resource_id=repo,
        # Never an IP source for a code-scanning alert.
        source_ip=None,
        metadata=metadata,
...
def _resolve_credentials(integration: Integration) -> dict[str, Any]:
    ...
    Mirrors github_secret_scanning_ingestion_service. Installation tokens are
...
def ingest_github_code_scanning_alerts(
    *,
    ...
    """Ingest GitHub code-scanning alerts for one integration. Never raises.
    ...
    try:
        ...
        logger.warning("github_code_scanning: credential resolution failed (not logged)")
    ...
    try:
        alerts = connector.list_code_scanning_alerts(credentials, max_alerts=cap)
    except AuthenticationError:
        summary["succeeded"] = True
        summary["permission_limited"] = True
        summary["error_message"] = (
            "GitHub code-scanning access is not permitted for this token/repo "
            "(requires GitHub Advanced Security and security-events read scope)."
        )
        return summary
        ...
        if code in (403, 404):
            ...
            summary["permission_limited"] = True
            summary["error_message"] = (
                "GitHub code scanning is unavailable for this repository "
                "(feature disabled, private repo without Advanced Security, or "
                "insufficient token scope)."
            )
            return summary
        summary["error_message"] = "GitHub code-scanning request failed."
        return summary
        ...
        logger.exception("github_code_scanning: unexpected ingestion error")
        summary["error_message"] = "Unexpected error during code-scanning ingestion."
        return summary
    ...
    skipped = 0
    for alert in alerts:
        normalized = normalize_code_scanning_alert(alert, repo_full_name=repo_full_name)
        ...
        try:
            ...
            logger.warning("github_code_scanning: failed to upsert one alert; continuing")
            continue
        if outcome == "inserted":
            inserted += 1
        else:
