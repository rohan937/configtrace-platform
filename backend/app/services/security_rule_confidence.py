...
RULE_CONFIDENCE: dict[str, tuple[str, str]] = {
    ...
    "kubernetes_privileged_container": (HIGH, "Only fires when securityContext.privileged is explicitly true; unknown/omitted is never flagged."),
    "kubernetes_privileged_host_access": (HIGH, "Only fires on a deterministic combination of privileged_container_count>0 AND explicit hostPID/hostIPC/runtime-socket-mount/high-risk-capability evidence on the same workload record."),
    "kubernetes_root_container": (MEDIUM, "Only fires when runAsUser is explicitly set to 0; unset/unknown is never flagged. Root alone does not imply privileged access."),
    "kubernetes_run_as_non_root_disabled": (HIGH, "Only fires when runAsNonRoot is explicitly false; unset/unknown is never flagged."),
    "kubernetes_privilege_escalation_allowed": (HIGH, "Only fires when allowPrivilegeEscalation is explicitly true; unset/unknown is never flagged."),
    "kubernetes_dangerous_linux_capability": (HIGH, "Only fires when a capability is present in the curated dangerous-capability allowlist; severity is tiered by capability but confidence is uniformly high (exact string match, not a keyword guess)."),
    "kubernetes_all_capabilities_added": (HIGH, "Only fires when the literal capability 'ALL' is present in capabilities.add."),
    "kubernetes_host_pid_enabled": (HIGH, "Only fires when hostPID is explicitly true on the workload spec."),
    "kubernetes_host_ipc_enabled": (HIGH, "Only fires when hostIPC is explicitly true on the workload spec."),
    "kubernetes_host_network_enabled": (MEDIUM, "hostNetwork=true is common for legitimate system/CNI components, so medium confidence applies."),
    "kubernetes_dangerous_hostpath": (HIGH, "Only fires when a mounted hostPath matches a curated dangerous-path category (root/etc/proc/sys/var_run/kubelet_dir); the raw path string is never read."),
    "kubernetes_container_runtime_socket_mounted": (HIGH, "Only fires when a mounted hostPath matches the docker_socket or containerd_socket category specifically."),
    "kubernetes_seccomp_unconfined": (HIGH, "Only fires when the seccomp profile is explicitly categorized Unconfined; omitted/unknown is never flagged."),
    "kubernetes_apparmor_unconfined": (HIGH, "Only fires when the AppArmor profile is explicitly categorized unconfined; omitted/unknown is never flagged."),
    "kubernetes_writable_root_filesystem": (MEDIUM, "readOnlyRootFilesystem=false is the Kubernetes default for many images, so medium confidence applies."),
    "kubernetes_mutable_image_tag": (MEDIUM, "'latest' (explicit or implicit) is a common and sometimes intentional practice in non-production namespaces."),
    "kubernetes_service_account_token_automount": (MEDIUM, "automountServiceAccountToken=true is the Kubernetes default, so medium confidence applies; only an explicit true fires."),
    "kubernetes_sensitive_host_port": (HIGH, "Only fires when a hostPort matches the curated sensitive/infra port allowlist; reachability is never claimed."),
    "kubernetes_cluster_admin_binding": (HIGH, "Only fires when the binding's resolved role is exactly cluster-admin or an equivalent full wildcard; unresolved bindings never fire."),
    "kubernetes_unauthenticated_cluster_admin": (HIGH, "Only fires when cluster_admin_binding is true AND the subject is explicitly anonymous or the unauthenticated group."),
    "kubernetes_authenticated_group_cluster_admin": (HIGH, "Only fires when cluster_admin_binding is true AND the subject is explicitly the system:authenticated group."),
    "kubernetes_all_service_accounts_cluster_admin": (HIGH, "Only fires when cluster_admin_binding is true AND the subject is exactly the system:serviceaccounts group."),
    "kubernetes_wildcard_rbac_permissions": (HIGH, "Only fires when wildcard_permission_binding is explicitly true and the binding is not already a cluster-admin binding (avoids double-counting)."),
    "kubernetes_rbac_bind_permission": (HIGH, "Only fires when 'bind' is present in the subject's high_risk_permission_categories, derived from an exact verb/resource match."),
    "kubernetes_rbac_escalate_permission": (HIGH, "Only fires when 'escalate' is present in high_risk_permission_categories, derived from an exact verb/resource match."),
    "kubernetes_rbac_impersonate_permission": (HIGH, "Only fires when 'impersonate' is present in high_risk_permission_categories."),
    "kubernetes_service_account_token_creation": (HIGH, "Only fires when 'token_creation' (serviceaccounts/token create) is present in high_risk_permission_categories."),
    "kubernetes_secret_read_permission": (HIGH, "Only fires when 'secret_read' or 'secret_read_broad_scope' is present in high_risk_permission_categories; Secret values are never read."),
    "kubernetes_secret_write_permission": (HIGH, "Only fires when 'secret_write' is present in high_risk_permission_categories; Secret values are never read."),
    "kubernetes_pod_exec_permission": (HIGH, "Only fires when 'pod_exec' is present in high_risk_permission_categories."),
    "kubernetes_pod_attach_permission": (HIGH, "Only fires when 'pod_attach' is present in high_risk_permission_categories."),
    "kubernetes_broad_workload_creation": (HIGH, "Only fires when 'workload_write' is present in high_risk_permission_categories."),
    "kubernetes_rbac_modification_permission": (HIGH, "Only fires when 'role_or_cluster_role_write' or 'cluster_role_binding_write' is present in high_risk_permission_categories."),
    "kubernetes_admission_webhook_modification_permission": (HIGH, "Only fires when 'admission_webhook_write' is present in high_risk_permission_categories."),
    "kubernetes_crd_modification_permission": (HIGH, "Only fires when 'crd_write' is present in high_risk_permission_categories."),
    "kubernetes_public_load_balancer": (HIGH, "Only fires on a confirmed external_load_balancer exposure_category (assigned LoadBalancer address), never on requested/pending status."),
    "kubernetes_sensitive_nodeport": (MEDIUM, "NodePort reachability depends on node network placement, which the connector does not observe, so medium confidence applies."),
    "kubernetes_public_ingress_without_tls": (HIGH, "Only fires when public_exposure_category is confirmed external AND tls_covered is explicitly false."),
    "kubernetes_hostless_catchall_ingress": (HIGH, "Only fires when both catch_all_route and hostless host_category are explicitly true on the same rule."),
    "kubernetes_public_gateway_listener": (HIGH, "Only fires on a confirmed external_load_balancer public_exposure_category derived from an assigned external Gateway address."),
    "kubernetes_network_policy_allows_all_ingress": (HIGH, "Only fires when allows_all_ingress is explicitly true, derived from an empty/absent 'from' restriction."),
    "kubernetes_network_policy_allows_all_egress": (HIGH, "Only fires when allows_all_egress is explicitly true, derived from an empty/absent 'to' restriction."),
    "kubernetes_public_ipv4_cidr_allowed": (HIGH, "Only fires when public_ipv4_cidr_allowed is explicitly true (an unrestricted or 0.0.0.0/0-equivalent ipBlock)."),
    "kubernetes_public_ipv6_cidr_allowed": (HIGH, "Only fires when public_ipv6_cidr_allowed is explicitly true (an unrestricted or ::/0-equivalent ipBlock)."),
    "kubernetes_namespace_no_network_policy": (MEDIUM, "Absence of any NetworkPolicy is the common Kubernetes default, so medium confidence applies; never fires when NetworkPolicy collection was only partial."),
    "kubernetes_namespace_no_ingress_isolation": (MEDIUM, "Absence of ingress isolation is a common default state, so medium confidence applies; never fires when collection was only partial."),
    "kubernetes_namespace_no_egress_isolation": (MEDIUM, "Absence of egress isolation is a common default state, so medium confidence applies; never fires when collection was only partial."),
    "kubernetes_validating_webhook_fail_open": (HIGH, "Only fires when failurePolicy is explicitly 'Ignore' on a validating webhook."),
    "kubernetes_mutating_webhook_fail_open": (HIGH, "Only fires when failurePolicy is explicitly 'Ignore' on a mutating webhook."),
    "kubernetes_broad_admission_webhook": (MEDIUM, "Wildcard operation/apiGroup/resource rules are sometimes an intentional broad-coverage design, so medium confidence applies."),
    "kubernetes_admission_webhook_external_http": (HIGH, "Only fires when the connector explicitly resolved the webhook client URL scheme to http://; https:// and in-cluster Service clients never fire."),
    "kubernetes_psa_privileged_enforcement": (HIGH, "Only fires when the enforce label is explicitly 'privileged'; unset/unknown is never flagged."),
    "kubernetes_psa_enforcement_missing": (MEDIUM, "An unset PSA enforce label is a common cluster default, so medium confidence applies; never fires when PSA collection was only partial."),
    "kubernetes_psa_invalid_enforcement": (HIGH, "Only fires when the enforce label value does not match any recognized PSA level (privileged/baseline/restricted)."),
    "kubernetes_psa_weak_with_privileged_workloads": (HIGH, "Only fires on the deterministic combination of a privileged/host-access workload present AND PSA enforcement explicitly privileged, unset, or invalid in the same namespace."),
    "kubernetes_namespace_weak_governance": (MEDIUM, "An aggregate multi-signal rollup (PSA + NetworkPolicy + quota + privileged identity); requires at least 3 of 4 weak signals simultaneously, but is inherently a broader inference than a single-field rule."),
    "kubernetes_privileged_identity_in_weak_namespace": (MEDIUM, "Combines a high-privilege ServiceAccount signal with weak NetworkPolicy coverage and zero ResourceQuota — a cross-record rollup inference."),
    "kubernetes_privileged_workload_without_isolation": (MEDIUM, "Combines a privileged-workload signal with weak/absent NetworkPolicy coverage — a cross-record rollup inference."),
    "kubernetes_namespace_resource_governance_missing": (MEDIUM, "Absence of any ResourceQuota/LimitRange is a common default for smaller clusters, so medium confidence applies; never fires when quota/limit collection was only partial."),
    # Okta (message 6 of 8)
    "okta_super_admin_assigned": (HIGH, "Only fires when has_super_admin is explicitly true on the derived privileged-identity record."),
    "okta_high_tier_admin_assigned": (HIGH, "Only fires when has_high_privilege is explicitly true and has_super_admin is not — direct, deterministic tier evidence."),
    "okta_custom_admin_role_high_risk": (HIGH, "Only fires when a custom role's permission-derived privilege_tier is explicitly critical or high; unknown permission sets never fire."),
    "okta_admin_role_broad_resource_set": (MEDIUM, "Depends on resolving a resource set's resource list to a scope category; a legitimate broad delegation is a plausible normal configuration."),
    "okta_unscoped_admin_role_assignment": (MEDIUM, "An unscoped grant of a normally-scoped role type is common in smaller tenants, so medium confidence applies."),
    "okta_deprovisioned_identity_retains_admin_privilege": (HIGH, "Only fires on the explicit combination of user_status=DEPROVISIONED and a known (non-unknown) privilege tier."),
    "okta_suspended_identity_retains_admin_privilege": (HIGH, "Only fires on the explicit combination of user_status=SUSPENDED and a known privilege tier."),
    "okta_dormant_privileged_identity": (MEDIUM, "Staleness is derived from message-2's last_login_category bucket, not a precise measurement, and may reflect legitimate infrequent admin use."),
    "okta_never_used_privileged_identity": (MEDIUM, "A privileged account that has never signed in may simply be newly provisioned, so medium confidence applies."),
    "okta_privileged_group_grants_super_admin": (HIGH, "Only fires when the derived privileged-group's highest_privilege_tier is explicitly critical."),
    "okta_privileged_group_grants_high_tier_admin": (HIGH, "Only fires when highest_privilege_tier is explicitly high and not critical (mutually exclusive with the Super Admin group rule)."),
    "okta_broad_privileged_group": (MEDIUM, "Combines a privilege-tier signal with a membership-count bucket; broad legitimate admin groups exist in larger tenants."),
    "okta_signon_mfa_not_required": (HIGH, "Only fires when mfa_requirement_category is explicitly 'none'; unknown/missing is never flagged."),
    "okta_signon_mfa_optional": (HIGH, "Only fires when mfa_requirement_category is explicitly 'optional'."),
    "okta_broad_allow_rule_without_mfa": (HIGH, "Only fires on the deterministic combination of access=ALLOW, scope=all_users, and mfa_requirement=none on the same rule record; supersedes the generic no-MFA rule for this exact record."),
    "okta_phishing_resistant_not_required": (MEDIUM, "A lack of phishing-resistant requirement is a common, often acceptable posture for many rules, so medium confidence applies; never fires on unknown phishing-resistance."),
    "okta_weak_authenticator_enabled": (MEDIUM, "Enabling SMS/phone/email authenticators is frequently a legitimate account-recovery design choice, not itself proof of weak primary authentication."),
    "okta_password_policy_weak_min_length": (HIGH, "Only fires when password_min_length_category is explicitly 'weak' (<8 characters); unknown length is never flagged."),
    "okta_password_policy_no_lockout": (HIGH, "Only fires when password_lockout_present is explicitly false; unknown is never flagged."),
    "okta_password_policy_no_history": (HIGH, "Only fires when password_history_present is explicitly false; unknown is never flagged."),
    "okta_password_policy_no_complexity": (MEDIUM, "Modern guidance de-emphasizes composition rules relative to length, so this is treated as a lower-confidence, informational posture note."),
    "okta_oidc_wildcard_redirect": (HIGH, "Only fires when wildcard_redirect_present is explicitly true, derived from an exact '*' match in a redirect URI."),
    "okta_oidc_http_redirect": (HIGH, "Only fires when http_redirect_count is an explicit positive integer; unknown/unavailable redirect posture is never flagged."),
    "okta_oidc_custom_scheme_redirect_non_native": (MEDIUM, "App-type categorization can be imprecise for some client configurations, so medium confidence applies."),
    "okta_saml_response_signing_disabled": (HIGH, "Only fires when saml_response_signed is explicitly false; unknown/absent signOn configuration is never flagged."),
    "okta_saml_assertion_signing_disabled": (HIGH, "Only fires when saml_assertion_signed is explicitly false."),
    "okta_weak_token_endpoint_auth": (HIGH, "Only fires when token_endpoint_auth_method_category is explicitly 'none'."),
    "okta_app_assigned_to_everyone_group": (HIGH, "Only fires when the assigned group's everyone_group flag is explicitly true, derived from Okta's own built-in Everyone group identity."),
    "okta_deprovisioned_user_retains_app_assignment": (HIGH, "Only fires when the denormalized user_status on the assignment record is explicitly DEPROVISIONED."),
    "okta_suspended_user_retains_app_assignment": (HIGH, "Only fires when the denormalized user_status on the assignment record is explicitly SUSPENDED."),
    # Microsoft Entra ID (message 6 of 8)
    "entra_global_admin_assigned": (HIGH, "Only fires on an exact match of the stable Global Administrator role template GUID; never inferred from display name."),
    "entra_privileged_role_administrator_assigned": (HIGH, "Only fires on an exact match of the stable Privileged Role Administrator role template GUID."),
    "entra_privileged_authentication_administrator_assigned": (HIGH, "Only fires on an exact match of the stable Privileged Authentication Administrator role template GUID."),
    "entra_high_tier_admin_assigned": (HIGH, "Only fires when privilege_tier is explicitly 'high' and the assignment did not already match a more specific critical-role rule."),
    "entra_guest_global_admin": (HIGH, "Only fires on the explicit combination of guest=true and has_global_admin=true on the same derived privileged-identity record."),
    "entra_guest_has_high_privilege": (HIGH, "Only fires on the explicit combination of guest=true and has_high_privilege=true, excluding identities already matched by the Global Administrator rule."),
    "entra_disabled_guest_retains_high_privilege": (HIGH, "Only fires on the explicit combination of account_enabled_category=disabled, guest=true, and a known (non-unknown) privilege tier."),
    "entra_disabled_identity_retains_admin_privilege": (HIGH, "Only fires on the explicit combination of account_enabled_category=disabled and a known privilege tier; unknown tier never fires."),
    "entra_group_has_global_admin": (HIGH, "Only fires when the derived privileged-group's highest_privilege_tier is explicitly critical."),
    "entra_group_has_high_privilege": (HIGH, "Only fires when highest_privilege_tier is explicitly high and not critical."),
    "entra_guest_member_in_privileged_group": (HIGH, "Only fires on the explicit combination of a positive guest_member_count and a critical/high privilege tier on the same group record."),
    "entra_privileged_group_broad_membership": (MEDIUM, "Combines a privilege-tier signal with a membership-count threshold; broad legitimate admin groups exist in larger tenants."),
    "entra_service_principal_has_critical_privilege": (HIGH, "Only fires when the derived privileged-service-principal's highest_privilege_tier is explicitly critical."),
    "entra_service_principal_has_high_privilege": (HIGH, "Only fires when highest_privilege_tier is explicitly high and not critical."),
    "entra_disabled_service_principal_retains_privilege": (HIGH, "Only fires on the explicit combination of account_enabled=false and a known privilege tier."),
    "entra_service_principal_can_manage_directory_roles": (HIGH, "Only fires on an exact resolved Graph permission value match (RoleManagement.ReadWrite.Directory) against a confirmed Microsoft Graph resource."),
    "entra_service_principal_can_manage_app_role_assignments": (HIGH, "Only fires on an exact resolved Graph permission value match (AppRoleAssignment.ReadWrite.All) against a confirmed Microsoft Graph resource."),
    "entra_service_principal_can_grant_arbitrary_permissions": (HIGH, "Only fires on an exact resolved Graph permission value match (Policy.ReadWrite.PermissionGrant) against a confirmed Microsoft Graph resource."),
    "entra_service_principal_has_application_management_permission": (HIGH, "Only fires on an exact resolved Graph permission value match (Application.ReadWrite.All)."),
    "entra_service_principal_can_modify_conditional_access": (HIGH, "Only fires on an exact resolved Graph permission value match (Policy.ReadWrite.ConditionalAccess)."),
    "entra_service_principal_can_modify_authentication_methods": (HIGH, "Only fires on an exact resolved Graph permission value match (UserAuthenticationMethod.ReadWrite.All)."),
    "entra_service_principal_has_directory_write_permission": (HIGH, "Only fires on an exact resolved Graph permission value match (Directory.ReadWrite.All)."),
    "entra_service_principal_has_user_write_permission": (MEDIUM, "User.ReadWrite.All is a common, often legitimate automation permission; medium confidence applies."),
    "entra_service_principal_has_group_write_permission": (MEDIUM, "Group.ReadWrite.All is a common, often legitimate automation permission; medium confidence applies."),
    "entra_tenant_wide_critical_delegated_consent": (HIGH, "Only fires when consent_type_category is explicitly AllPrincipals and highest_scope_privilege_tier is explicitly critical."),
    "entra_tenant_wide_high_risk_delegated_consent": (HIGH, "Only fires when consent_type_category is explicitly AllPrincipals and highest_scope_privilege_tier is explicitly high."),
    "entra_user_scoped_critical_consent": (MEDIUM, "A single user's own consent has a smaller blast radius than tenant-wide consent, so medium confidence applies even for a critical-tier scope."),
    "entra_user_scoped_high_risk_consent": (MEDIUM, "A single user's own consent has a smaller blast radius than tenant-wide consent."),
    "entra_external_unverified_app_tenant_wide_consent": (MEDIUM, "A composite of three signals (tenant-wide consent, high/critical scope tier, unverified publisher); publisher verification status alone does not indicate malicious intent."),
    "entra_ca_broad_access_without_mfa": (HIGH, "Only fires on the deterministic combination of state=enabled, block_access!=true, mfa_requirement=not_required, and coverage=all_users_all_apps on the same policy record."),
    "entra_ca_access_without_mfa": (HIGH, "Only fires on the explicit combination of state=enabled, block_access!=true, and mfa_requirement=not_required; unknown MFA requirement never fires."),
    "entra_ca_mfa_optional_within_grant_controls": (MEDIUM, "An OR grant-control operator with MFA as one of several options may reflect an intentional, adequately-protective policy design."),
    "entra_ca_legacy_auth_not_blocked": (HIGH, "Only fires on the explicit combination of state=enabled, legacy_auth_targeted=true, and block_access!=true — a positive weak-posture signal, never inferred from absence."),
    "entra_ca_report_only_broad_protection": (MEDIUM, "Report-only policies are often intentionally staged before enforcement; medium confidence applies."),
    "entra_weak_authentication_method_enabled": (MEDIUM, "Enabling SMS/voice authentication methods is frequently a legitimate account-recovery design choice, not itself proof of weak primary authentication."),
    "entra_authentication_strength_not_phishing_resistant": (MEDIUM, "Scoped to custom authentication strengths only; whether the strength is actually relied upon by an enforced Conditional Access policy is not verified by this single-record rule."),
    "entra_application_wildcard_redirect": (HIGH, "Only fires when has_wildcard_redirect is explicitly true, derived from an exact wildcard match in a redirect URI."),
    "entra_application_http_redirect": (HIGH, "Only fires when web_has_http_redirect is explicitly true."),
    "entra_application_custom_scheme_redirect_unexpected": (MEDIUM, "Absence of a public-client redirect URI is a reasonable but imperfect proxy for 'not a native client'."),
    "entra_application_expired_credential": (HIGH, "Only fires when nearest_credential_expiry_category is explicitly 'expired'; unknown expiry is never flagged."),
    "entra_service_principal_expired_credential": (HIGH, "Only fires when nearest_credential_expiry_category is explicitly 'expired'."),
    "entra_service_principal_assignment_not_required": (MEDIUM, "Whether disabling assignment-required actually broadens access depends on the application's own authorization behavior."),
    "entra_disabled_user_retains_application_assignment": (HIGH, "Only fires when the denormalized account_enabled_category on the assignment record is explicitly 'disabled'."),
    "entra_dynamic_group_assigned_to_application": (MEDIUM, "Dynamic group membership is a common, often intentional automation pattern."),
    "entra_role_assignable_group_assigned_to_application": (MEDIUM, "Role-assignable groups are frequently used for legitimate administrative delegation; combining that with an application assignment is a plausible normal configuration."),
    # Snowflake (message 6 of 8)
    "snowflake_user_accountadmin": (HIGH, "Only fires on the exact boolean has_accountadmin from message 5's effective-privilege closure; never inferred from role name."),
    "snowflake_service_user_accountadmin": (HIGH, "Same exact-field trigger as snowflake_user_accountadmin, scoped to user_type in (service, service_agent)."),
    "snowflake_user_securityadmin": (HIGH, "Only fires on the exact boolean has_securityadmin; suppressed when ACCOUNTADMIN already fired for the same user."),
    "snowflake_user_can_manage_grants": (HIGH, "Only fires on the exact boolean has_manage_grants; suppressed when ACCOUNTADMIN/SECURITYADMIN already fired."),
    "snowflake_user_sysadmin_or_useradmin": (MEDIUM, "Fires on has_sysadmin/has_useradmin gated to tier=='medium' exactly; a broader or unknown tier does not fire this rule."),
    "snowflake_disabled_privileged_user": (HIGH, "Only fires when disabled=='disabled' (explicit tri-state) and tier is exactly critical/high; unknown disabled state never fires."),
    "snowflake_legacy_service_user_privileged": (MEDIUM, "Depends on both the LEGACY_SERVICE user-type classification and the derived tier being high/critical — a compound but deterministic condition."),
    "snowflake_legacy_service_user": (MEDIUM, "LEGACY_SERVICE is a documented transitional/deprecated type, but its mere presence does not by itself indicate active risk without privilege context."),
    "snowflake_custom_role_manage_grants": (HIGH, "Only fires on the exact boolean has_manage_grants for a role_category=='custom' role; never inferred from the role's name."),
    "snowflake_custom_role_manage_grants_identity_admin": (HIGH, "Combines two exact derived signals (has_manage_grants and an explicit identity_administration category) on the same record."),
    "snowflake_custom_role_high_privilege": (MEDIUM, "Depends on message 5's custom-role tier derivation from observed grants, which is deterministic but a coarser signal than a single boolean flag."),
    "snowflake_role_controls_managed_access_schema": (HIGH, "Only fires on an explicit owns_managed_access_schema_count > 0 for a custom role, cross-referenced against message 3's own managed-access schema set."),
    "snowflake_role_owns_security_integration_high_privilege": (MEDIUM, "Requires both an explicit ownership count and a high/critical tier gate — a deliberately conservative composite to avoid ordinary-admin noise."),
    "snowflake_role_owns_storage_integration_high_privilege": (MEDIUM, "Same composite-gating rationale as the security-integration variant."),
    "snowflake_role_owns_external_access_integration_high_privilege": (MEDIUM, "Same composite-gating rationale as the security-integration variant."),
    "snowflake_role_owns_authentication_policy_high_privilege": (MEDIUM, "Same composite-gating rationale as the security-integration variant."),
    "snowflake_role_owns_network_policy_high_privilege": (MEDIUM, "Same composite-gating rationale as the security-integration variant."),
    "snowflake_high_privilege_role_owns_database": (MEDIUM, "Database ownership is routine Snowflake administration; only the high/critical-tier custom-role composite is flagged."),
    "snowflake_future_ownership_grant": (HIGH, "Only fires on an explicit future_ownership_count > 0 — a directly observed future-grant record, not an inference."),
    "snowflake_public_future_ownership_grant": (HIGH, "Only fires on an explicit future_public_ownership_count > 0, derived from already-collected future object-grant records."),
    "snowflake_public_future_write_access": (HIGH, "Only fires on an explicit future_public_write_count > 0."),
    "snowflake_public_future_data_access": (HIGH, "Only fires on an explicit future_public_read_count > 0."),
    "snowflake_public_future_broad_privilege": (MEDIUM, "Fires on a residual count (total minus categorized read/write/ownership) — real but less specific evidence of exactly which privilege is involved."),
    "snowflake_network_policy_allows_anywhere": (HIGH, "Only fires on the explicit 0.0.0.0/0 or ::/0 sentinel check; unknown broad-access state never fires."),
    "snowflake_mfa_optional_with_password": (MEDIUM, "Depends on set_on=='ACCOUNT' plus mfa_enrollment=='optional' plus password appearing in authentication_methods — a compound but deterministic condition."),
    "snowflake_mfa_optional_for_person_auth": (MEDIUM, "Depends on set_on=='ACCOUNT' plus mfa_enrollment=='optional'; gated to account-wide policies so a service-user-scoped policy is never mistaken for human MFA weakness."),
    "snowflake_mfa_password_only_scope": (MEDIUM, "REQUIRED_PASSWORD_ONLY is still an MFA requirement (narrower than REQUIRED, not absent) — flagged as an advisory gap, not a hard failure."),
    "snowflake_scim_critical_privilege_run_as": (MEDIUM, "Depends on message 6's SCIM run-as role-name resolution against the role-hierarchy closure — deterministic but a multi-step derived resolution rather than a single direct field."),
    "snowflake_scim_high_privilege_run_as": (MEDIUM, "Same resolution-chain rationale as the critical-tier variant."),
    "snowflake_saml_integration_incomplete_config": (MEDIUM, "Config-completeness gaps can reflect an integration mid-setup, not necessarily an active security weakness."),
    "snowflake_user_high_risk_future_grant": (MEDIUM, "Depends on an aggregated count field crossing zero; real evidence but coarser than an exact boolean."),
    "sentry_active_organization_owner": (HIGH, "Only fires on the exact combination org_role_category=='owner' and member_status_category=='active'; unknown role/status never fires."),
    "sentry_active_organization_manager": (HIGH, "Only fires on the exact combination org_role_category=='manager' and member_status_category=='active'."),
    "sentry_active_organization_admin": (HIGH, "Only fires on the exact combination org_role_category=='admin' and member_status_category=='active'."),
    "sentry_pending_privileged_invitation": (HIGH, "Only fires on the exact combination member_status_category=='pending' and a known privileged org_role_category; unknown role never fires."),
    "sentry_member_broad_routing_authority": (MEDIUM, "Combines two derived count fields on an ordinary member; a legitimate on-call/triage member is a plausible normal configuration."),
    "sentry_member_team_admin_without_org_role": (MEDIUM, "Team Admin delegation to an ordinary member is a common, often legitimate administrative pattern in smaller organizations."),
    "sentry_team_has_broad_routing_authority": (MEDIUM, "Combines two derived count fields on a team; broad legitimate team-based routing exists in larger organizations."),
    "sentry_team_has_unresolved_members": (MEDIUM, "An unresolved membership can reflect a very recent removal not yet reconciled, not necessarily a stale/broken configuration."),
    "sentry_metric_alert_unrouted": (HIGH, "Only fires when status_category is explicitly 'enabled' and action_count is exactly 0 (not unknown/None)."),
    "sentry_issue_alert_unrouted": (HIGH, "Only fires when status_category is explicitly 'enabled' and action_count is exactly 0 (not unknown/None)."),
    "sentry_metric_alert_disabled_with_routing_configured": (MEDIUM, "A disabled rule with retained actions is frequently an intentional pause, so this is a lower-priority advisory rather than a strong risk signal."),
    "sentry_issue_alert_disabled_with_routing_configured": (MEDIUM, "Same rationale as the metric-alert variant."),
    "sentry_alert_targets_missing_team": (HIGH, "Only fires when target_resolved is explicitly false AND the record's own completeness is FAMILY_COMPLETE AND the owning rule is confirmed enabled — never on a partial/unknown inventory."),
    "sentry_alert_targets_missing_member": (HIGH, "Same completeness/enabled-gating discipline as the missing-team variant."),
    "sentry_alert_references_inactive_member": (HIGH, "Only fires when target_resolved is explicitly true and target_active is explicitly false on a confirmed-enabled rule."),
    "sentry_alert_references_disabled_integration": (HIGH, "Only fires when integration_status_category is explicitly 'disabled' on a confirmed-enabled rule's action."),
    "sentry_ownership_targets_missing_team": (HIGH, "Same completeness/enabled-gating discipline as the alert-routing missing-team variant, applied to ownership rules."),
    "sentry_ownership_targets_missing_member": (HIGH, "Same completeness/enabled-gating discipline as the alert-routing missing-member variant, applied to ownership rules."),
    "sentry_ownership_targets_inactive_member": (HIGH, "Only fires when target_resolved is explicitly true and target_active is explicitly false on an active ownership rule."),
    "sentry_repository_pending_deletion": (HIGH, "Only fires on the repository's own explicit status_category value; unknown status never fires."),
}
