...
class DatadogActivitySignalGenerateRequest(BaseModel):
    """POST /security/datadog-activity/generate-signals request body (all optional)."""

    lookback_hours: int = Field(default=24, ge=1, le=168)
    max_signals: int = Field(default=100, ge=1, le=1000)


class DatadogActivitySignalGenerateResponse(BaseModel):
    """Datadog configuration activity signal generation summary."""
...
class DatadogCorrelationGenerateRequest(BaseModel):
    """POST /security/datadog-correlations/generate request body (all optional)."""

    lookback_hours: int = Field(default=24, ge=1, le=168)
    max_correlations: int = Field(default=100, ge=1, le=1000)


class DatadogCorrelationGenerateResponse(BaseModel):
    """Datadog Risk × Activity correlation generation summary."""
