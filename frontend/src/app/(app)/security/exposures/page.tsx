...
const SEVERITY_COLOR: Record<SecurityFindingSeverity, string> = {
  critical: "#e84040",
  high: "#f5632a",
  medium: "#f5a623",
  low: "#6b9cf8",
...
const SEVERITY_LABEL: Record<SecurityFindingSeverity, string> = {
  critical: "Critical",
  high: "High",
  medium: "Medium",
  low: "Low",
...
function RemediationBlock({ remediation }: { remediation: Record<string, unknown> }) {
  const summary = typeof remediation.summary === "string" ? remediation.summary : null;
  const steps = Array.isArray(remediation.steps)
    ? (remediation.steps as unknown[]).filter((s) => typeof s === "string")
    : [];
  // Any other keys render as key-value (masked when secret-like).
  ...
  return (
    <div>
      ...
      {summary ? (
        <div style={{ fontSize: "13px", color: "#c4c8d4", lineHeight: 1.6, marginTop: "6px" }}>
          {summary}
        </div>
      ) : null}
      {steps.length > 0 ? (
