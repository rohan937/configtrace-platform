...
export const DEMO_SCRIPT_INCIDENT: DemoStep[] = [
  {
    ...
    talkTrack:
      "Start by loading a sample incident from the Cases page — GitHub, AWS, Cloudflare, Vercel, or Supabase (the latest provider-expansion example, covering database access posture). Each demo seeds one coherent, clearly-marked evidence chain (a configuration risk, related activity, signals, correlations, and a case) on a hidden demo integration. Nothing connects to a real provider.",
    whatToClick: "On Cases, use the demo banner to load the GitHub, AWS, Cloudflare, Vercel, or Supabase incident demo.",
