...
export const DEMO_SCRIPT_INCIDENT: DemoStep[] = [
  {
    ...
    talkTrack:
      "Start by loading a sample incident from the Cases page — GitHub, AWS, Cloudflare, Vercel, Supabase, Firebase, Stripe, Shopify, Azure, Google Cloud, Twilio, SendGrid, Auth0, Datadog, Clerk, PagerDuty, Linear, Jira, GitLab, or Terraform Cloud (with Datadog covering an outbound webhook integration review — missing secret headers, non-HTTPS endpoint, and auth material in custom headers; Clerk covering a session policy review — extended lifetime, disabled token rotation, and disabled reverification; PagerDuty covering an incident-response webhook subscription review — non-HTTPS delivery, broad event scope, and secret not indicated; Linear covering a webhook configuration review — no signing secret, non-HTTPS delivery, and broad resource scope; Jira covering a permission-scheme and webhook posture review; GitLab covering a branch protection and CI/CD variable posture review; Terraform Cloud covering a workspace auto-apply, global remote state, and variable posture review; no API keys, raw URLs, webhook secrets, routing keys, integration keys, emails, user IDs, IP addresses, variable values, state files, or team identities in any demo). Each demo seeds one coherent, clearly-marked evidence chain (a configuration risk, related activity, signals, correlations, and a case) on a hidden demo integration. Nothing connects to a real provider.",
    whatToClick: "On Cases, use the demo banner to load the GitHub, AWS, Cloudflare, Vercel, Supabase, Firebase, Stripe, Shopify, Azure, Google Cloud, Twilio, SendGrid, Auth0, Datadog, Clerk, PagerDuty, Linear, Jira, GitLab, or Terraform Cloud incident demo.",
