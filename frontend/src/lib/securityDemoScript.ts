...
export const DEMO_SCRIPT_INCIDENT: DemoStep[] = [
  {
    ...
    talkTrack:
      "Start by loading a sample incident from the Cases page — GitHub, AWS, Cloudflare, Vercel, Supabase, Firebase, Stripe, Shopify, Azure, Google Cloud, Twilio, SendGrid, Auth0, Datadog, or Clerk (with Datadog covering an outbound webhook integration review — missing secret headers, non-HTTPS endpoint, and auth material in custom headers; Clerk covering a session policy review — extended lifetime, disabled token rotation, and disabled reverification; no API keys, raw URLs, webhook secrets, emails, user IDs, or IP addresses in either demo). Each demo seeds one coherent, clearly-marked evidence chain (a configuration risk, related activity, signals, correlations, and a case) on a hidden demo integration. Nothing connects to a real provider.",
    whatToClick: "On Cases, use the demo banner to load the GitHub, AWS, Cloudflare, Vercel, Supabase, Firebase, Stripe, Shopify, Azure, Google Cloud, Twilio, SendGrid, Auth0, Datadog, or Clerk incident demo.",
